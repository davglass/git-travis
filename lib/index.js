...
var fetch = function(user, repo, branch, callback) {
    travis.repos(user, repo).builds().get(function(err, res) {
        if (err) {
            return callback(err);
        }
...
exports.print = function(user, repo, branch, callback) {
    ...
    var onFetch = function(err, data) {
        ...
        travis.repos(user, repo).builds(item.id).get(function(err, res) {
            if (err) {
    ...
    travisAuthWithUsernamePassword = function (callback) {
        prompt.start();
        prompt.get({
            properties: {
                username: {
    ...
    travisAuthWithToken = function (callback) {
